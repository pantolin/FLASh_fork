...
class GAMG(AMG):
    """Same assembled system as AMG, but uses PETSc's geometric-algebraic
    multigrid (gamg) preconditioner instead of SOR. Inherits solve() and
    get_solution() from AMG; only the preconditioner is swapped after the
    Assembler has built the KSP.
    """
    ...
    def _setup_extra(self):
        ...
        # override the SOR preconditioner set inside the Assembler with gamg
        ksp = self.assembler.A_sover
        pc = ksp.getPC()
        pc.setType('gamg')
        ksp.setUp()
