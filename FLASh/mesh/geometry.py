...
class BezierElement:
    ...
    def evaluate_A(
        ...
        a2 = np.einsum('nkl,nps,nqs->npkql', kron, ijf, ijf)
    ...
    def evaluate_arclen(
        ...
        jf = self.evaluate_jacobian(x)
        arclen = np.linalg.norm(jf, axis=1)
...
class SplineGeometry:
    ...
    def __init__(
        ...
        element_basis_x, elements_x = get_basis_support(knots[0], self.degree)
        element_basis_y, elements_y = get_basis_support(knots[1], self.degree)
        ...
        self.create_coarse_mesh(opts.get("periodic", False))


    @staticmethod
    def create_spline(
        knots: list[int],
        control_points: np.ndarray[np.float64],
        levelset,
        opts: dict = None
    ) -> Self:
        ...
        degree = opts.get("spline_degree", 2)
        ...
        return SplineGeometry(knots, basis, control_points, levelset, opts)
    ...
    def get_bezier_element_from_point(self, x) -> BezierElement:
    ...
    def create_coarse_mesh(self, periodic = False) -> None:

        pts = [
            np.unique(np.array(self.knots[0])), 
            np.unique(np.array(self.knots[1]))    
        ]

        self.coarse_mesh = CoarseMesh.create_cartesian_mesh(pts, periodic)
    ...
    def plot_det(
        ...
        for x0, x1 in zip(knots_x[self.degree:-self.degree-1], knots_x[self.degree+1:-self.degree]):
            for y0, y1 in zip(knots_y[self.degree:-self.degree-1], knots_y[self.degree+1:-self.degree]):
                ...
                contour = ax.contourf(X, Y, det, cmap='viridis')
    ...
    def plot_arclen(
        ...
        for x0, x1 in zip(knots_x[self.degree:-self.degree-1], knots_x[self.degree+1:-self.degree]):
            for y0, y1 in zip(knots_y[self.degree:-self.degree-1], knots_y[self.degree+1:-self.degree]):
                ...
                det = bezier_element.evaluate_arclen(X)[:,0]
                det = det.reshape(10, 10)
        ...
        for x0, x1 in zip(knots_x[self.degree:-self.degree-1], knots_x[self.degree+1:-self.degree]):
            for y0, y1 in zip(knots_y[self.degree:-self.degree-1], knots_y[self.degree+1:-self.degree]):
                ...
                det = bezier_element.evaluate_arclen(X)[:,1]
                det = det.reshape(10, 10)
