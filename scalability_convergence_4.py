...
if __name__ == "__main__":            
    ...
    for i in range(1, n_max):
        ...
        stats_8.append(bddc_solver(geometry, parameter_function, elasticity, communicators, opts = opts))
        ...
        stats_10.append(bddc_solver(geometry, parameter_function, elasticity, communicators, opts = opts))
        ...
        stats_12.append(bddc_solver(geometry, parameter_function, elasticity, communicators, opts = opts))

    if communicators.global_comm.Get_rank() == 0:

        x = [8*(np.arange(1,n_max)**2)] * 3

        iterations = [
            [stat["local iterations"] for stat in stats_8],
            [stat["local iterations"] for stat in stats_10],
            [stat["local iterations"] for stat in stats_12]
        ]

        condition_numbers = [
            [stat["condition number"] for stat in stats_8],
            [stat["condition number"] for stat in stats_10],
            [stat["condition number"] for stat in stats_12]
        ]
