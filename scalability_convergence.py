...
if __name__ == "__main__":            
    ...
    configs = [
        ...
        {"levelset": fischer_koch_s,  "epsilon_min": 0.6, "epsilon_max": 1.5, "stats": [], "label": "Fischer Koch S"},
        {"levelset": schoen,          "epsilon_min": 0.9, "epsilon_max": 1.5, "stats": [], "label": "Schoen"},
    ...
    for i in range(1, n_max):
        ...
        for config in configs:
            ...
            stats.append(bddc_solver(geometry, parameter_function, elasticity, communicators, opts = opts))

    if communicators.global_comm.Get_rank() == 0:

        x = [8*(np.arange(1,n_max)**2)] * len(configs)
        ...
        for config in configs:
            ...
            iterations.append([stat["local iterations"] for stat in stats])
            condition_numbers.append([stat["condition number"] for stat in stats])
