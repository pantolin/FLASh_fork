...
class GlobalDofsManager:
    ...
    def __init__(self, geometry: dict, coarse_mesh: dolfinx.mesh.Mesh, subdomains: list[Subdomain], parameters, linear_pde, communicators):
    ...
    def create_rectangle(
        ...
        coarse_mesh = create_2D_mesh(N, communicators.self_comm, P0, P1)

        if communicators.global_comm.Get_rank() == 0:
            V = dolfinx.fem.functionspace(coarse_mesh, ("Lagrange", 1))
            fh = dolfinx.fem.Function(V)
            fh.name = "fh"
            fh.interpolate(parameter_function)
            parameters = fh.x.array
        else:
            parameters = None  

        parameters = communicators.global_comm.bcast(parameters, root=0)

        x = coarse_mesh.geometry.x
        top = coarse_mesh.topology
        ...
        N = c_2_v.num_nodes
        process_subdomains = np.arange(rank, N, size)
        ...
        return GlobalDofsManager(geometry, coarse_mesh, subdomains, parameters, linear_pde, communicators)
    ...
    def plot(
        ...
        if communicators.global_comm.Get_rank() == 0:
            ...
            for s_id in range(N):
                ...
                impl_func = levelset(list(parameters[vertices]), p0, p1)
    ...
    def _create_boundary_dofs(self) -> None:
        ...
        for s_ind, s_id in enumerate(self.process_subdomains):
            ...
            vertices_dofs = self.subdomains[s_ind].get_vertices_dofs()
        ...
        for s_ind, s_id in enumerate(self.process_subdomains):
            ...
            edges_dofs = self.subdomains[s_ind].get_interior_edges_dofs()
        ...
        top.create_connectivity(0, 2)
        v_2_c = top.connectivity(0, 2)

        top.create_connectivity(2, 0)
        c_2_v = top.connectivity(2, 0)

        n_vertices = v_2_c.num_nodes
        ...
        for vertex_id in range(n_vertices):
            cells = v_2_c.links(vertex_id)
            
            local_ids = [np.where(c_2_v.links(cell) == vertex_id)[0][0] for cell in cells]
        ...
        n_edges = e_2_c.num_nodes

        self._edges_dofs_ranges = np.empty((n_edges, 2), dtype=np.int32)

        for edge_id in range(n_edges):
            cells = e_2_c.links(edge_id)
            local_edge_id  = np.where(c_2_e.links(cells[0]) == edge_id)[0]

            if len(cells) > 1:
                reference = num_edges[cells[0], local_edge_id]
                for cell in cells[1:]:
                    local_id = np.where(c_2_e.links(cell) == edge_id)[0]
    ...
    def _create_dirichlet_boundary_conditions(self) -> None:
        ...
        for (type_, fun, marker, tag) in self.linear_pde.exterior_bc:
            ...
            if type_ == 0:
                ...
                vertices = dolfinx.fem.locate_dofs_geometrical(self.primal_V, marker)
                ...
                local_indices = [np.where(c_2_v.links(s) == v)[0][0] for v, s in zip(vertices, subdomains)]
        ...
        for (type_, fun, marker, tag) in self.linear_pde.exterior_bc:
            ...
            if type_ == 0:
                ...
                edges = locate_entities(self.coarse_mesh, 1, marker)
                edges_dirichlet_dofs.append(edges)
                edd_tag.append(np.full(edges.shape, tag, dtype = np.int32))

                subdomains = [e_2_c.links(e)[0] for e in edges]
                local_indices = [np.where(c_2_e.links(s) == e)[0][0] for e, s in zip(edges, subdomains)]
        ...
        for s_ind, s_id in enumerate(self.process_subdomains):
            ...
            for tag in range(max_tag + 1):
                ...
                u = subdomain.get_projected_function(fun, boundary=True)
                v_dofs = subdomain.get_vertices_dofs()
                e_dofs = subdomain.get_interior_edges_dofs()
    ...
    def _create_boundary_scaling(self) -> None:
        ...
        for s_ind, s_id in enumerate(self.process_subdomains):
            ...
            vertices = c_2_v.links(s_id)
            edges = c_2_e.links(s_id)

            subdomain = self.subdomains[s_ind]
            local_vertices = subdomain.get_vertices_dofs()
            local_edges = subdomain.get_interior_edges_dofs()
    ...
    def create_R(self, subdomain_id: int) -> SparseMatrix:
        ...
        vertices = c_2_v.links(subdomain_id)
        edges = c_2_e.links(subdomain_id)
        ...
        R = scipy.sparse.csr_matrix(
            (values, (rows, cols)), shape=(cols.size, self.get_num_boundary_dofs())
        )
    ...
    def create_Rc(self, subdomain_id: int) -> SparseMatrix:
        ...
        Rc = scipy.sparse.csr_matrix(
            (values, (rows, cols)), shape=(cols.size, self.get_num_primals())
        )
    ...
    def transform_to_lagrange(self, us) -> list[npt.NDArray[np.float64]]:

        l_us = []

        for u, subdomain in zip(us, self.subdomains):
            l_us.append(subdomain.get_lagrange_function(u))
    ...
    def plot_solution(self, us) -> None:
        ...
        us = self.transform_to_lagrange(us)
